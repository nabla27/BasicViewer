    ...
    : QTableWidget(parent)
{
    ...
    {//ショートカットキーの登録
        ...
        connect(scCtrC, &QShortcut::activated, this, &ReTableWidget::copyCell);
        connect(scCtrV, &QShortcut::activated, this, &ReTableWidget::pasteCell);
        connect(scCtrX, &QShortcut::activated, this, &ReTableWidget::cutCell);
    }
    {//contextMenuの設定
        setContextMenuPolicy(Qt::CustomContextMenu);
        connect(this, &ReTableWidget::customContextMenuRequested,
                this, &ReTableWidget::onCustomContextMenu);
        normalMenu = new QMenu(this);
        /* copy */
        QAction *actCopy = new QAction("copy", normalMenu);
        normalMenu->addAction(actCopy);
        connect(actCopy, &QAction::triggered, this, &ReTableWidget::copyCell);
        /* cut */
        QAction *actCut = new QAction("cut", normalMenu);
        normalMenu->addAction(actCut);
        connect(actCut, &QAction::triggered, this, &ReTableWidget::cutCell);
        /* paste */
        QAction *actPaste = new QAction("paste", normalMenu);
        normalMenu->addAction(actPaste);
        connect(actPaste, &QAction::triggered, this, &ReTableWidget::pasteCell);
        /* delete */
        QAction *actDelete = new QAction("delete", normalMenu);
        normalMenu->addAction(actDelete);
        connect(actDelete, &QAction::triggered, this, &ReTableWidget::deleteCell);
        /* insert */
        ...
        connect(actInsertRowUp, &QAction::triggered, this, &ReTableWidget::insertRowUp);
        connect(actInsertRowDw, &QAction::triggered, this, &ReTableWidget::insertRowDown);
        connect(actInsertColLf, &QAction::triggered, this, &ReTableWidget::insertColLeft);
        connect(actInsertColRg, &QAction::triggered, this, &ReTableWidget::insertColRight);
        ...
        connect(actReverseRow, &QAction::triggered, this, &ReTableWidget::reverseRow);
        connect(actReverseCol, &QAction::triggered, this, &ReTableWidget::reverseCol);
        ...
        connect(actPlot, &QAction::triggered, this, &ReTableWidget::plot);
        connect(actPlotClip, &QAction::triggered, this, &ReTableWidget::gnuplotClip);
        ...
        connect(act3DBar, &QAction::triggered, this, &ReTableWidget::visualize3DBar);
        connect(act3DScatter, &QAction::triggered, this, &ReTableWidget::visualize3DScatter);
        /* export */
        ...
        connect(actLatexCode, &QAction::triggered, this, &ReTableWidget::toLatexCode);
...
/* tableにデータをセット */
template <>
void ReTableWidget::setData(const QList<QList<QString> >& data)
{
...
/* table全体のデータを取得 */
template <>
QList<QList<QString> > ReTableWidget::getData() const
{
...
/* tableの選択されているデータを取得 */
template<>
QList<QList<QList<float> > > ReTableWidget::selectedData() const
{
    /* 選択されれるセルの範囲を取得 */
...
void ReTableWidget::onCustomContextMenu(const QPoint& point)
{
    normalMenu->exec(viewport()->mapToGlobal(point));
}

/* コピー */
void ReTableWidget::copyCell()
{
    /* tableの情報を取得 */
...
/* 選択されたセルの上に行を追加 */
void ReTableWidget::insertRowUp()
{
    insertRow(currentIndex().row());
}

/* 選択されたセルの下に行を追加 */
void ReTableWidget::insertRowDown()
{
    insertRow(currentIndex().row() + 1);
}

/* 選択されたセルの左に列を追加 */
void ReTableWidget::insertColLeft()
{
    insertColumn(currentIndex().column());
}

/* 選択されたセルの右に列を追加 */
void ReTableWidget::insertColRight()
{
    insertColumn(currentIndex().column() + 1);
}

/* 選択された範囲の行を上下対称に入れ替える */
void ReTableWidget::reverseRow()
{
    /* 選択された範囲を取得 */
    const QList<QTableWidgetSelectionRange> selectedRangeList = selectedRanges();
...
void ReTableWidget::gnuplotClip()
{
    /* 選択された範囲 */
    const QList<QTableWidgetSelectionRange> selectedRangeList = selectedRanges();
    if(selectedRangeList.size() < 1) { return; }
...
void ReTableWidget::visualize3DBar()
{

}


void ReTableWidget::visualize3DScatter()
{

}

/* 選択された範囲のセルを表として描画するlatexコードをクリップボードに貼り付け */
void ReTableWidget::toLatexCode()
{
    /* 選択された範囲を取得 */
    const QList<QTableWidgetSelectionRange> selectedRangeList = selectedRanges();
