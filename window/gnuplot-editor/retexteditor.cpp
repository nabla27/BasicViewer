...
ReTextEdit::ReTextEdit(QWidget *parent)
    : QPlainTextEdit(parent)
{
    {
        connect(this, &ReTextEdit::blockCountChanged,
                this, &ReTextEdit::updateLineNumberAreaWidth);
        connect(this, &ReTextEdit::updateRequest,
                this, &ReTextEdit::updateLineNumberArea);
        connect(this, &ReTextEdit::cursorPositionChanged,
                this, &ReTextEdit::highlightLine);
    }
    {
    ...
    {//ハイライトの初期設定
        textHighlight = new ReSyntaxHighlighter(document());
    }
}
...
void ReTextEdit::changeCompleterModel()
{
...
void ReTextEdit::setCompleter(QCompleter *completer)
{
    ...
    QObject::connect(c, QOverload<const QString&>::of(&QCompleter::activated),
                     this, &ReTextEdit::insertCompletion);
}

QCompleter* ReTextEdit::completer() const
{
    return c;
}

void ReTextEdit::insertCompletion(const QString &completion)
{
...
QString ReTextEdit::textUnderCursor() const
{
    QString text = currentCmd;
    return text.remove('\'').remove('\"');
}

void ReTextEdit::focusInEvent(QFocusEvent *e)
{
...
/* 括弧の補完 */
void ReTextEdit::bracketCompletion(QKeyEvent *e, const QChar nextChar)
{
...
void ReTextEdit::keyPressEvent(QKeyEvent *e)
{
...
/* 行番号の表示幅を決定 */
int ReTextEdit::lineNumberAreaWidth()
{
    /* 桁数digitsを求める */
    int digits = 1;
...
/* 行番号の表示幅を設定 */
void ReTextEdit::updateLineNumberAreaWidth(int)
{
    setViewportMargins(lineNumberAreaWidth(), 0, 0, 0);
}

/* スクロールなどに応じて行番号表示エリアを設定 */
void ReTextEdit::updateLineNumberArea(const QRect& rect, int dy)
{
    if(dy)
...
void ReTextEdit::resizeEvent(QResizeEvent *e)
{
...
/* 行表示エリアの番号表示 */
void ReTextEdit::lineNumberAreaPaintEvent(QPaintEvent *event)
{
