...
CmdLine::CmdLine(QWidget *parent)
    : QLineEdit(parent)
{
    {//レイアウト色の設定
...
void CmdLine::setCompleter(QCompleter *completer)
{
    if(c)
        c->disconnect(this);
    ...
    QObject::connect(c, QOverload<const QString&>::of(&QCompleter::activated),
                     this, &CmdLine::insertCompletion);
}


void CmdLine::insertCompletion(const QString &completion)
{
    if(c->widget() != this)
        return;
...
QString CmdLine::textUnderCursor() const
{
    return this->text().split(' ').constFirst();
}

void CmdLine::focusInEvent(QFocusEvent *e)
{
    if(c)
        c->setWidget(this);
...
void CmdLine::keyPressEvent(QKeyEvent *e)
{
